...
const task = document.querySelector(".task_body");

addnewtask.onclick = createTodo;
  ...
  if (taskInput.value.length == 0) {
    ...
    taskList.innerHTML += `
        ...
        <div class="right_side side">
            <button class="donebtn"><svg width="16" height="12" viewBox="0 0 16 12" fill="black" xmlns="http://www.w3.org/2000/svg">
    ...
    for (let i = 0; i < close.length; i++) {
      close[i].onclick = function () {
        let taskbody = this.parentElement.parentElement;
        taskbody.style.display = "none";
      };
    }
  }
  const checkStatus = document.querySelectorAll(".donebtn");

  checkStatus.forEach((btn) => {
    btn.onclick = foo1;
  });
